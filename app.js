...
// Watch directory for changes then perform some action
            require('chokidar').watch('./photos', {ignored: /[\/\\]\./}).on('add', function (event, photoPath) {
                ...
                var watermarkX = 0;
                var watermarkY = 0;
                var watermarkPath = requestBody.watermarkFilePath;
                ...
                function addWatermarkToPhoto(command) {
                    gm(__dirname + '/photos/' + basename)
                        ...
                        .write('' + __dirname + '/waterMarkedPhotos/' + basename, function (e) {
                            ...
                            console.log(command)
                            console.log(__dirname + '/photos/' + basename)
                            if (requestBody.twitterActive != null && requestBody.twitterActive == 'on') {
                                postMediaToTwitter();
                            }
                            if (requestBody.facebookActive != null && requestBody.facebookActive== 'on' ) {
                                postMediaToFacebook();
                            }
                        });
                }
                    ...
                    T.post('media/upload', {media_data: b64content}, function (err, data, response) {
                        ...
                        T.post('media/metadata/create', meta_params, function (err, data, response) {
                            ...
                            if (!err) {
                                ...
                                var params = {status: requestBody.twitterHashtag, media_ids: [mediaIdStr]}
                                ...
                                if (response && !response.error) {
                                    for (var name in response.data) {
                                        if (response.data.hasOwnProperty(name)) {
                                            ...
                                            if (val.name == requestBody.facebookAlbumName) {
                                                FB.api('' + val.id + '/photos', 'post', {
                                                    source: fs.createReadStream(watermarkDirectoryPath + basename),
                                                    caption: requestBody.facebookCaption
                                                }, function (res) {
                                                    if (!res || res.error) {
                                                        console.log(!res ? 'error occurred' : res.error);
                                                        return;
